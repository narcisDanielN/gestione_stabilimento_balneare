...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Port-Out (Driven Ports)}: definiscono i requisiti infrastrutturali necessari al core per funzionare (es. \autoref{lst:Repository}).
    \end{itemize}
...
Grazie a questa struttura, il sistema gode di una straordinaria resilienza: l'utilizzo delle interfacce assicura che il \textbf{disaccoppiamento} sia totale. Nel nostro caso, gli adattatori contenuti nel package \texttt{infrastructure.persistence.jdbc} contengono il codice specifico per interagire con il database MySQL.
\newline Se in futuro fosse necessario migrare verso un diverso sistema di persistenza o un servizio cloud, sarebbe sufficiente sviluppare un nuovo Adapter senza dover modificare una singola riga di codice nel Core applicativo.
...
A integrazione dell'architettura esagonale, è stato adottato l'approccio \textbf{DDD-lite}. Mentre l'esagonale definisce i confini, il DDD stabilisce che il nucleo del progetto deve riflettere il linguaggio e le regole della realtà modellata. 
